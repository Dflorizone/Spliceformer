...
class SpliceAI_10K(nn.Module):
    ...
    def forward(self, features):
        ...
        for i,residualUnit in enumerate(self.res_layers):
            x = residualUnit(x)
            skip = skip + self.skip_layers[i+1](x)
            #skip = torch.cat([skip,self.skip_layers[i+1](x)],axis=1)
...
class SpliceAI_10K_FullInit(nn.Module):
    def __init__(self, CL_max, crop=True, n_channels=32, activation='relu', **kwargs):
        super().__init__()
        self.CL_max = CL_max
        self.crop = crop
        self.n_channels = n_channels
        ...
        self.res_W = [11, 11, 21, 41]
        self.res_dilation = [1, 4, 10, 25]
        self.kernel_size = 1

        # Activation helper
        self.activations = nn.ModuleDict({
            'relu': nn.ReLU(inplace=False),
            'leaky_relu': nn.LeakyReLU(0.01, inplace=False),
            'selu': nn.SELU(inplace=True),
            'none': nn.Identity()
        })

        # First conv
        self.conv_layer_1 = nn.Conv1d(4, n_channels, kernel_size=self.kernel_size, stride=1)

        # Skip connections
        self.skip_layers = nn.ModuleList([
            nn.Conv1d(n_channels, n_channels, kernel_size=self.kernel_size, stride=1)
            for _ in range(5)
        ])

        # ResComboBlocks
        self.res_layers = nn.ModuleList()
        for i in range(4):  # 4 ResComboBlocks
            combo = nn.ModuleList()
            for _ in range(4):  # each combo has 4 ResidualBlocks
                pad = int(self.res_dilation[i] * (self.res_W[i] - 1) / 2)
                block = nn.ModuleDict({
                    'convlayer1': nn.Conv1d(n_channels, n_channels, kernel_size=self.res_W[i],
                                            stride=1, dilation=self.res_dilation[i], padding=pad),
                    'bn1': nn.BatchNorm1d(n_channels, momentum=0.01),
                    'convlayer2': nn.Conv1d(n_channels, n_channels, kernel_size=self.res_W[i],
                                            stride=1, dilation=self.res_dilation[i], padding=pad),
                    'bn2': nn.BatchNorm1d(n_channels, momentum=0.01),
                    'shortcut': nn.Identity()
                })
                combo.append(block)
            self.res_layers.append(nn.Sequential(*combo))  # This mirrors ResComboBlock

        # Final conv
        self.conv_final = nn.Conv1d(n_channels, 3, kernel_size=self.kernel_size, stride=1)
    ...
    def forward(self, x):
        ...
        for i, combo in enumerate(self.res_layers):
            for block in combo:
                residual = block['shortcut'](x)
                x = self.activations[self.activation](block['bn1'](x))
                x = block['convlayer1'](x)
                x = self.activations[self.activation](block['bn2'](x))
                x = block['convlayer2'](x)
                x = x + residual
            skip = skip + self.skip_layers[i + 1](x)
